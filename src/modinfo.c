 ...
 */
static sr_error_info_t *
sr_module_oper_data_update(struct sr_mod_info_mod_s *mod, sr_lock_mode_t oper_mode, sr_sid_t sid, sr_conn_ctx_t *conn,
        ...
        sr_error_info_t **cb_error_info)
{
    ...
    if (!(opts & SR_OPER_NO_STORED)) {
        ...
        if ((err_info = sr_module_file_oper_data_load(mod, oper_mode, conn->cid, sid, &diff))) {
            return err_info;
        }
    ...
    if (!mod_info->data_cached) {
        ...
        if (mod_info->ds == SR_DS_OPERATIONAL) {
            ...
            if (mod->state & MOD_INFO_WLOCK) {
                oper_mode = SR_LOCK_WRITE;
            } else if (mod->state & MOD_INFO_RLOCK_UPGR) {
                oper_mode = SR_LOCK_READ_UPGR;
            } else {
                assert(mod->state & MOD_INFO_RLOCK);
                oper_mode = SR_LOCK_READ;
            }
            if ((err_info = sr_module_oper_data_update(mod, oper_mode, sid, conn, request_xpath, timeout_ms, opts,
    ...
    for (i = 0; i < mod_info->mod_count; ++i) {
        ...
        if (mod->state & MOD_INFO_CHANGED) {
            if (mod_info->ds == SR_DS_OPERATIONAL) {
                ...
                if ((err_info = sr_module_file_oper_data_load(mod, SR_LOCK_WRITE, mod_info->conn->cid, sid, &diff))) {
                    goto cleanup;
                }
                ...
                if (mod_info->ds == SR_DS_RUNNING) {
                    ...
                    oper_mode = ((mod_info->ds2 == SR_DS_OPERATIONAL) && (mod->state & MOD_INFO_RLOCK2) ? SR_LOCK_READ :
                            SR_LOCK_NONE);
                    if ((err_info = sr_module_file_oper_data_load(mod, oper_mode, mod_info->conn->cid, sid, &diff))) {
                        goto cleanup;
                    }
